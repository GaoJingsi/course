<template>
    <div class="pagination" role="group" aria-label="分页">
        ...
        <button v-for="(p, index) in pages" :key="index" :id="'page-' + p"
                type="button" class="btn btn-default btn-white btn-round"
                :class="{'btn-primary active':page == p}"
                @click="selectPage(p)">
            {{p}}
        </button>
        <button type="button" class="btn btn-default btn-white btn-round"
...
<script>
    export default {
        name: 'pagination',
        props: {
            // 显示的页码数，比如总共有100页，只显示10页，其它用省略号表示
            itemCount: {
                type: Number,
                default: 5
            },
        ...
        methods: {
            ...
            render(page) {
                let _this = this;
                _this.page = page;
                _this.pageTotal = Math.ceil(_this.total / _this.size);
                _this.pages = _this.getPageItems(_this.pageTotal, _this.page, _this.itemCount || 5);
            },
            ...
            selectPage(page) {
                ...
                if (this.page !== page) {
                    _this.page = page;
                    _this.$emit('list', page, _this.size)
                }
            },
